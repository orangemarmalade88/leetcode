...
public class StoneGameIII {
	 ...
	 * winner is the player with the highest score and there could be a tie. The
	...
	public String stoneGameIII(int[] stoneValue) {
		...
		for (int i = n - 1; i >= 0; i--) {
			int res = sum[i] - dp[i + 1];
			if (i + 2 <= n)
				res = Math.max(res, sum[i] - dp[i + 2]);
			if (i + 3 <= n)
				res = Math.max(res, sum[i] - dp[i + 3]);
			dp[i] = res;
		}

		if (2 * dp[0] > sum[0])
			return "Alice";
		else if (2 * dp[0] == sum[0])
			return "Tie";
		else
			return "Bob";
